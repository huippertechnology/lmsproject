...
const Resizer = () => {
   ...
   const resizeNodeRef = useRef<HTMLElement | null>(null);
   ...
   const nodeState = useEditorState({
      editor,
      selector: (ctx) => {
         if (!ctx.editor.isFocused || !ctx.editor.isEditable) {
            return null;
         }
   ...
   const handleResize = (event: PointerEvent) => {
      const node = resizeNodeRef.current;
      const resizeInfo = resizeInfoRef.current;

      if (!node || !resizeInfo.isResizing) {
         return;
      }
   ...
   useEffect(() => {
      const node = nodeState?.node;
      resizeNodeRef.current = node ?? null;
   ...
   if (!nodeState || !contentElement.current) {
      return;
   }
   ...
   return createPortal(
      <div ref={controlRef} className="rte-resizer">
         {renderResizerHandle('nw-resize', 0, {
            width: 12,
            left: -10,
            top: -10,
         })}
         ...
         {renderResizerHandle('sw-resize', 1, {
            width: 12,
            right: -10,
            top: -10,
         })}
