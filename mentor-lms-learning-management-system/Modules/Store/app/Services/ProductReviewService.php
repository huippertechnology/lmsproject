...
use Modules\Store\Models\ProductReview;

class ProductReviewService extends MediaService
{
    public function totalReviews(string $productId): array
    {
        $reviews = ProductReview::ofProduct($productId)
            ->select('rating')
            ->get();
    ...
    public function getReviews(array $data): LengthAwarePaginator|Collection
    {
        ...
        $reviews = ProductReview::searchWhen('review', $data, 'reviews_search')
    ...
    public function userReview(string $productId, string $userId): ?ProductReview
    {
        return ProductReview::ofProduct($productId)->ofUser($userId)->first();
    }
    ...
    {
        return ProductReview::findOrFail($id)->update($data);
    }
