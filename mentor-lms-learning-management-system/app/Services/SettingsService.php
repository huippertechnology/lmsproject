...
{
    public function __construct(protected SiteTranslationService $siteTranslationService) {}
    ...
    public function getNavbar(string $slug)
    {
        return DB::transaction(function () use ($slug) {
            $navbar = Navbar::whereSlug($slug)
                ->whereActive(true)
                ->with('navbarItems')
                ->first();

            if ($navbar) {
                $this->siteTranslationService->applyToNavbarItems($navbar->navbarItems);
            }
    ...
    {
        return DB::transaction(function () use ($slug) {
            $footer = Footer::whereSlug($slug)
                ->whereActive(true)
                ->with('footerItems')
                ->first();

            if ($footer) {
                $this->siteTranslationService->applyToFooterItems($footer->footerItems);
            }
